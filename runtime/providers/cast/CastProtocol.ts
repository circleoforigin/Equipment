export const CAST_PROTOCOL_VERSION =
  0

export const CAST_PAYLOAD_TYPE_STRING =
  0
...
export function decodeCastMessage(
  buffer: Buffer,
): CastMessage {
  let offset = 0
  ...
  while (
    ...
    throw new Error(
      `Unsupported Cast protobuf wire type: ${wireType}.`,
    )
  }
  ...
  if (
    ...
    throw new Error(
      'Cast protobuf message is incomplete.',
    )
  }
...
export function frameCastMessage(
  message: CastMessage,
): Buffer {
  const payload =
    encodeCastMessage(
