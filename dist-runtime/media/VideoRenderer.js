...
export async function renderImageToVideo(imagePath, options = {}) {
    if (!ffmpegPath) {
    ...
    const frameRate = options.frameRate ??
        DEFAULT_FRAME_RATE;
    const workingDirectory = await mkdtemp(join(tmpdir(), 'settingforge-equipment-video-'));
    const outputPath = join(workingDirectory, 'display.mp4');
    /*
     * scale:
...
async function runFfmpeg(executablePath, args) {
